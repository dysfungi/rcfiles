...
- The fast pre-commit gate runs `--fail-slow 0.75` as a drift guard. It catches selected tests that exceed 0.75s, but an existing test drifting from <0.2s to 0.2–0.75s is not caught until duration regeneration. `@pytest.mark.slow` remains the manual force-slow override.
