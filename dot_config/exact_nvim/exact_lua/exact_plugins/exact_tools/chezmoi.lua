...
return {
  ...
  init = function()
    -- Async chezmoi integration: replaces chezmoi.nvim's synchronous watch()
    -- and apply() calls, which block the editor for 2.7-6s (chezmoi
    -- status/apply on this repo exceeds plenary's 5s default timeout).
    --
    -- Defined inside init so requires run after Lazy has loaded plugins.
    -- State tables (_active/_pending) live here and are shared across all
    -- autocmd firings via closure.
    --
    -- Pattern: single-flight + queue-latest, keyed by source_path.
    --   - If no job is running for a file, start immediately.
    --   - If a job is running, stash the new request (replacing any prior
    --     pending request — only the latest matters).
    --   - On job exit, pop and start the pending request if any.
    --
    -- Future considerations:
    --   - vim.throttle()/vim.debounce() (neovim/neovim#33179, Mar 2025, not
    --     merged as of nvim 0.12.2): when available, evaluate as a built-in
    --     for the time-gating aspect.
    --   - lazy.util.throttle: provides single-flight + queue-one, but (a)
    --     coroutine-based (requires bridging to plenary Job callbacks), (b) no
    --     per-key support (single global `pending` flag), (c) no argument
    --     passing (our apply needs per-buffer source_path).
    --   - Why hand-rolled: completion-driven queuing (wait for process exit,
    --     then start next), not time-driven. No existing library fits cleanly.
