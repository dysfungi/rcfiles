...
--   - vim.throttle()/vim.debounce() (neovim/neovim#33179, proposed Mar 2025,
--     not merged as of nvim 0.12.2): when available, may replace the time-
--     gating aspect of this queue.
...
--     (our apply needs per-buffer source_path). The time-based delay component
--     is interesting for rate-limiting rapid saves but doesn't address the
--     core job-completion-driven queuing need.
--   - Why hand-rolled: this pattern is job-completion-driven (wait for process
--     exit, then start next), not time-driven. Existing libraries are either
--     timer-based, coupled to specific async runtimes, or archived.
local chezmoi_async = (function()
